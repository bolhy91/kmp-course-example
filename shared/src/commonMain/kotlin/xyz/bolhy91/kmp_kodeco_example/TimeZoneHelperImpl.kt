...
class TimeZoneHelperImpl : TimeZoneHelper {
    ...
    override fun search(startHour: Int, endHour: Int, timezoneStrings: List<String>): List<Int> {
        ...
        val currentTimeZone = TimeZone.currentSystemDefault()
// repase cada hora en el rango de tiempo
        for (hour in timeRange) {
            var isGoodHour = false
            // revisar cada zona horaria
            for (zone in timezoneStrings) {
                val timezone = TimeZone.of(zone)
                // verificar si esta bien la zona horaria
                if (timezone == currentTimeZone) {
                    continue
                }
                // verificar si la hora es valida
                if (!isValid(
                        timeRange = timeRange,
                        hour = hour,
        ...
        // devuelve el arreglo con las zonas horarias
        return goodHours
    }
    ...
    private fun isValid(
        ...
        // obtener el instante actual zona horaria utc
        val currentUTCInstant: Instant = Clock.System.now()
        // conviertir el instante utc a la zona horaria de destino
        val currentOtherDateTime: LocalDateTime = currentUTCInstant.toLocalDateTime(otherTimeZone)
        // Obtener un LocalDateTime con la hora indicada
